...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, destination, callback) {
  var self = this;
  ...
  .then(function(){
    ...
    if (destination)
      request['@Destination'] = self.options.entryPoint;
...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback) {
  ...
  if (self.options.skipRequestCompression)
    ...
    zlib.Gzip(request || response, requestToUrlHelper);
...
SAML.prototype.getAuthorizeUrl = function (req, callback) {
  var self = this;
  self.generateAuthorizeRequest(req, self.options.passive, self.options.destination, function(err, request){
    if (err)
      return callback(err);
...
SAML.prototype.getAuthorizeForm = function (req, callback) {
  ...
  self.generateAuthorizeRequest(req, self.options.passive, self.options.destination, function(err, request) {
    ...
    if (self.options.skipRequestCompression) {
      ...
      zlib.Gzip(request, getAuthorizeFormHelper);
    }
  });
